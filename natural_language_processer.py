...
class entity_linker(object):
    ...
    def __init__(self, spacy_doc):
        ...
        self.tickers        = []
        # embedder
        self.embedder       = foundation.SentenceTransformer(foundation.env.embedder_model, cache_folder = foundation.env.cache_dir)
        ...
        self.tickers_df = foundation.pd.read_csv(foundation.env.s_and_p_500_tickers_path)
...
class nlp_pipeline(object):
    ...
    def __init__(self):
        ...
        self.nlp = foundation.spacy.load(foundation.env.spacy_model)

        try:
            # try opening the processed file if one exists
            with open(foundation.env.processed_finance_news_filename, "r") as _file:
        ...
        # ----- exception management -----
            ...
            foundation.log(f"Processed finance news file '{foundation.env.processed_finance_news_filename}' not found! Ensure scraping is complete.\n cwd = {foundation.os.getcwd()}", self.__str__(), "error")
